...
async function main() {
  ...
  try {
    const res1 = await robustFetch('https://routine.zohirrayhan.me/get_time_slots?department=cse');
  ...
  try {
    const res2 = await robustFetch('https://routine.zohirrayhan.me/api/free-rooms?time=10:00-11:30&department=cse');
  ...
  try {
    const res3 = await robustFetch('https://routine.zohirrayhan.me/api/schedule', {
      method: 'POST',
      body: JSON.stringify({
        view_mode: 'room',
        room_number: 'KT-503',
  ...
  try {
    const res4 = await robustFetch('https://routine.zohirrayhan.me/api/schedule', {
      method: 'POST',
      body: JSON.stringify({
        view_mode: 'room',
        room_number: 'KT-201',
